...
geometry_msgs::Twist wall_following(double Kp, double Ki, double Kd)
{
    ...
    double wall_error = left_sonar - right_sonar;
    double wall_error_d = wall_error - wall_error_old;
    static double wall_error_sum = 0.0;  // Make it static to keep its value between calls
    wall_error_sum += wall_error;

    double steering_control = Kp * wall_error + Kd * wall_error_d + wall_error_sum * Ki;
